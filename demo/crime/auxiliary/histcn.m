...
for d=1:nd
    ...
    if isempty(ed)
        ed = DEFAULT_NBINS;
        throw(MException('histcn:incorrectSize','DEFAULT_NBINS detected'))
    end
    if isscalar(ed) % automatic linear subdivision
       ed = linspace(min(Xd),max(Xd),ed+1);
      %  ed = linspace(0,1,ed+1);
    end
    edges{d} = ed;
    % Call histc on this dimension
    [dummy loc(:,d)] = histc(Xd, ed, 1);
    % Use sz(d) = length(ed); to create consistent number of bins
    sz(d) = length(ed)-1; %%% changed by SRF to get rid of the empty bins at the edges.
end % for-loop

% Clean
clear dummy

% This is needed because points could hit the right border
% but I don't want it to be used for X and Y -- SRF
% what to do?
sz = max([sz; max(loc,[],1)]); %% changed by SRF
